...
async def login_for_access_token(
    ...
    # OAuth2PasswordRequestFrom uses "username" field, but treat it as email
    ...
    # verify user exists and password correctness, don't reveal which failed for best practice
...
async def forgot_password(
    ...
    if user:
        ...
        return {
            "message": "If an account exists with this email, you wil receive password reset instructions."
        }
...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]): # each request gets its own database session
